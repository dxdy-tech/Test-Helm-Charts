...
require_vars NAMESPACE KEYSTORE_SECRET_NAME MI_ADMIN_SECRET_NAME
...
# Example:
#   kubectl -n "${NAMESPACE}" create secret generic "${MI_ADMIN_SECRET_NAME}" \
#     --from-literal=username=admin \
#     --from-literal=password=<strong-password>
if ! kubectl -n "${NAMESPACE}" get secret "${MI_ADMIN_SECRET_NAME}" >/dev/null 2>&1; then
  echo "ERROR: Admin credentials secret '${MI_ADMIN_SECRET_NAME}' not found in namespace '${NAMESPACE}'." >&2
  echo "Create it manually before running deploy:" >&2
  echo "  kubectl -n ${NAMESPACE} create secret generic ${MI_ADMIN_SECRET_NAME} \\" >&2
  echo "    --from-literal=username=<username> \\" >&2
  echo "    --from-literal=password=<password>" >&2
  exit 1
fi
echo "Admin credentials secret ${MI_ADMIN_SECRET_NAME} is present in namespace ${NAMESPACE}."
