...
if [[ -f "${PRIMARY_KEYSTORE}" && -f "${INTERNAL_KEYSTORE}" && -f "${TRUSTSTORE}" && "${FORCE_KEYSTORE_REGEN:-false}" != "true" ]]; then
  echo "Keystores already exist at ${KEYSTORE_DIR}; skipping regeneration."
  exit 0
fi

rm -f "${PRIMARY_KEYSTORE}" "${INTERNAL_KEYSTORE}" "${TRUSTSTORE}" "${CERT_FILE}"
...
cp "${PRIMARY_KEYSTORE}" "${INTERNAL_KEYSTORE}"
...
echo "Generated ${PRIMARY_KEYSTORE}, ${INTERNAL_KEYSTORE}, and ${TRUSTSTORE}"
